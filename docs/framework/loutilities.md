...
- **`ManageLocalTables.update()`'s optional `lockfile` parameter serializes concurrent callers sharing a filesystem** — e.g. several gunicorn workers each independently calling `create_app()` → `update_local_tables()` at boot. Without it, two callers can each find no existing `LocalUser` row for the same user/interest pair and insert their own duplicate; an earlier version of the same lookup also missed rows that had gone inactive since the previous sync, for the same duplicate-row effect. Both fixed ([loutilities#103](https://github.com/louking/loutilities/issues/103), [loutilities#104](https://github.com/louking/loutilities/issues/104)); contractility, membertility, and routetility all pass a `lockfile` now and each ran a one-time migration to deduplicate rows the bugs had already created.
