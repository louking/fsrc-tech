...
- Local dev has moved from Windows-native to a WSL2 (Ubuntu) environment (migrated repo-by-repo through 2026-08), with Docker Desktop's WSL integration bridging the two — a container started from either side is reachable from both, since it's one shared engine. Two things stayed Windows-native by design: tmtility's hardware-reader client processes (see [tmtility](../applications/tmtility.md)), which need direct Windows access to USB/Bluetooth devices, and this wiki's own repo (low migration value)
