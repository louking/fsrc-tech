...
- **Testing**: `pytest` suite (`pytest.ini`/`conftest.py`, same pattern as contractility — `APP_NAME`/`APP_VER` env defaults before package import, since `rrwebapp/__init__.py` and `version.py` read them at module level) covers every top-level module importable under the bare-`Flask` test fixture: `model.py`, `resultsutils.py`, `clubmember.py`, `helpers.py`, `raceresults.py`, and the three external-results modules `athlinksresults.py`/`ultrasignupresults.py`/`runningaheadresults.py`. `tools.py`, despite being top-level rather than under `views/admin/`, isn't importable this way — it pulls in the same `celery.py` import chain via `views.admin.member`, so being outside `views/admin/` isn't sufficient on its own; each module has to be import-tested individually. Two pre-existing bugs surfaced by this coverage remain unfixed as out of scope: `ServiceAttributes.set_attr()` queries a model column that doesn't exist and is never actually called; and `AthlinksCollect.getresults()`'s age-group "fuzzy match" logic uses `(x/5)*5`-style checks written for Python 2's truncating division, so under Python 3 it silently never matches.
...
- **Config** is read from `/config/rrwebapp.cfg`, with secrets (DB password, RabbitMQ password) mounted as Docker secrets. The `app` container's crontab is mounted the same way, from `./config/cronjobs` — a runtime bind mount rather than baked into the image at build time, so the cron schedule can change without a rebuild.
...
- **`runtilities` needs `>=3.0.1`, not just `>=3.0.0`**: `runtilities==3.0.0`'s `running.runningahead` module had a module-level `import version` with no top-level `version` package to resolve it — always fails. Harmless for contractility/membertility (they never import `running.runningahead`, only `running.runsignup`), but fatal for scoretility, which imports it for its RunningAHEAD results-analysis integration, breaking every app startup. Fixed upstream in the `running` repo — the dead code was a leftover non-functional `main()` CLI stub (`argparse`'s `version=` kwarg was removed from Python 3 years ago, so that function could never run).
