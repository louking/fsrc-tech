...
- **Google Maps loads async, gated by an inline `onGmapsReady` stub**: the Maps script tag isn't in an asset bundle (bundled `<script>` tags can't carry `async`); each map-using template loads it directly with `loading=async&callback=gmapsCallback`, and the corresponding page JS defers its map-dependent init until both the DOM and that callback have fired. The `gmapsCallback`/`onGmapsReady` registration must be a tiny inline `<script>` ahead of the async tag, not defined inside the page's own JS bundle — that bundle loads behind the ~2MB shared common bundle and lost the race against the (much smaller) async Maps script in practice. Extending `google.maps.OverlayView` for the custom SVG overlay must use `Object.setPrototypeOf`, not prototype replacement, once that setup is deferred past parse time. Switching to async loading also exposed a second, independent race on the route detail page: Maps' own `onAdd()` callback and the route-data AJAX response can complete in either order, and the elevation chart's distance array used to only get computed as a side effect of `onAdd()` having already fired — fixed by computing it unconditionally as soon as route data arrives, instead of piggybacking it on map-render timing.
...
- **`pytest` suite** (previously none) covers `helpers.py`, `geo.py`'s `GmapsLoc` (mocked `googlemaps.Client`, no real network calls), `models.py`'s free functions/`update_local_tables()`, `files.py`, `locations.py`, `dbinit.py`, `nav.py`'s role-based menu construction, and — on the `DbCrudApiRolePermissions`-based admin views and the frontend `MethodView`s — the override methods that don't themselves call `render_template()` (`permission()`, `beforequery()`, `snaploc()`, etc.), called directly on the module-level view singletons rather than through the full DataTables request/response protocol. Several modules (`locations.py`, `nav.py`, the admin/frontend route/icon views) build a real `googlemaps.Client`/register with `flask_nav` at *module import time* using the app-level config, so they're only importable once `create_app()` has run once in the process — tests defer those imports via a fixture rather than a top-level `import`. Interleaving the full `app`/`dbapp` fixtures with the lightweight `bareapp`/`bare_dbapp` ones in the same test session also surfaced a `db.session` gotcha: `create_app()` rebinds Flask-SQLAlchemy's session proxy to whichever app called it last, so later tests using the other fixture pair got order-dependent failures — fixed with a `_rebind_session()` helper called between fixture types.
- **`libpass` must be installed *alongside* `passlib`, not instead of it**: a "security fixes" pass swapped `passlib==1.7.4` for `libpass==1.9.3` outright, but `libpass` 1.9.3 only ships a partial `passlib`-namespace shim (no `context.py`/`CryptContext`), breaking `flask_security.core`'s import entirely. Restored `passlib==1.7.4` alongside it, matching membertility's already-working combination.
