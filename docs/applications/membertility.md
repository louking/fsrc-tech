...
- **`setuptools>=83` dropped `pkg_resources` entirely**, breaking any dependency that still does an unconditional `from pkg_resources import ...`. Hit via `FormEncode==2.0.1` (pulled in by `loutilities.tables`); fixed by bumping to `FormEncode==2.1.1`, which uses `importlib.metadata` instead — no need to pin `setuptools` back down.
- **A local `.venv` at the repo root is treated as the source of truth for `requirements.txt`** when the Docker build's `pip install` hits a resolver conflict — reconcile against `pip list --format=freeze` from that venv rather than guessing a new pin. Hit when adding `pytest` (which needs `packaging>=22`) alongside an old `packaging==21.3` pin broke the build; fixed by adopting the venv's actual `packaging` version and adding two of pytest's transitive deps the file was missing.
