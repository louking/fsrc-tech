...
- **RunSignUp client now subclasses `running.runsignup.RunSignupBase`** (from the `runtilities` PyPI package, now a real dependency in `requirements.txt`) instead of vendoring the full client. `RunSignupBase` owns the shared auth/session plumbing (`open`/`close`, `_rsuget`/`_rsugetcsv`); contractility's subclass adds only what `running`'s own `RunSignUp` doesn't need — coupon management (`getcoupons`/`setcoupon`, POST-based) and participant lists, plus a local `_rsupost` since the base only implements GET. This inheritance only became viable once `running.runsignup_fluent.RunSignupFluent` (needing `universalclient`/`rauth`) was split into its own module — before that, importing `running.runsignup` at all would have dragged those in. Watch for a transitive-dependency gotcha: `running.runsignup` imports `loutilities.csvwt` at module level (for an unrelated CSV helper), which unconditionally imports `openpyxl` — contractility had to add `openpyxl` to `requirements.txt` purely to satisfy that import chain, not for any CSV feature of its own. Instantiation is centralized via `helpers.make_runsignup_client()` (reads `RSU_KEY`/`RSU_SECRET`/`RSU_API_REG_TOKEN`/`RSU_API_REG_SECRET` from config) — same pattern as `members`' equivalent helper. Endpoints target `api.runsignup.com/rest/...` (the Aug 2025 migration) and the client sends the `rsu_api_reg` token/`X-RSU-API-REG-SECRET` header RunSignUp will require on every call starting 2027-01-01. The legacy email/password Login API path (dead code — both call sites only ever passed `key=`/`secret=`) has been removed.
