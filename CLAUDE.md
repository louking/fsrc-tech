...
This applies at the single-bullet level too, not just whole-page duplication: a gotcha bullet can quietly balloon into method names, config variable names, and historical narrative that duplicates the source `CLAUDE.md` and drifts just as easily as a full section would. Keep gotcha bullets to a few sentences covering the fact and its consequence, not the how — link out for the rest. Seen concretely in `applications/contractility.md`'s RunSignUp gotcha, which grew to six sentences of implementation detail before being trimmed back (see `CHANGELOG.md` 2026-08-02).

**Most source repos have moved to a WSL2 (Ubuntu) dev environment** (see `docs/infrastructure/hosting.md#development`) — check `//wsl.localhost/Ubuntu/home/lking/projects/<repo-name>` first (a plain single-level path, e.g. `//wsl.localhost/Ubuntu/home/lking/projects/loutilities`, directly readable with Read/Glob/Grep/Bash from this Windows-side session) rather than reaching for `gh api` or `WebFetch` on GitHub. As of 2026-08, that covers `loutilities`, `caddy-docker`, `rrwebapp`, `contracts`, `members`, `runningroutes`, `logmon`, and `wordpress-docker`. **The old Windows-side checkouts of all eight are frozen and renamed `<repo-name>-obsolete`** (e.g. `...\projects\rrwebapp\rrwebapp-obsolete`, `...\projects\logmon\logmon-obsolete`) — don't read from them, they no longer receive updates. `tm-csv-connector` (Windows-native PyInstaller hardware-reader tools, deliberately not migrated) and `loudevprocess` still live only at the old Windows path, under `C:\Users\lking\Documents\Lou's Software\projects\<repo-name>`, nested one level deeper (e.g. `...\projects\tm-csv-connector\tm-csv-connector`, mirroring this repo's own `fsrc-tech\fsrc-tech` nesting) — `Glob` for the repo name first if a direct path 404s. Only fall back to GitHub if a repo isn't present in either location, or the task specifically needs the canonical remote state (e.g. checking what's actually pushed/merged). When a repo has no `CLAUDE.md`, reading the actual local source to find key classes/modules and their docstrings works well too — that's how `docs/framework/loutilities.md` was built. This pattern isn't limited to `docs/applications/`/`docs/framework/` pages — `docs/infrastructure/caddy.md` was built the same way from the `caddy-docker` repo.
...
**Verifying claims about third-party hardware/software behavior**: don't state specific technical parameters (timeout values, delays, algorithm mechanics) about vendor systems like Trident from memory or plausible-sounding inference — check the vendor's own docs first, even for a one-sentence gotcha. `docs/race-services/timing.md`'s Best Seen settling-time explanation was wrong twice in a row this way: first stated as a fixed "about half a second," then guessed as "a configurable tag timeout" — both corrected only after fetching Trident's [Tag Settings](https://www.manula.com/manuals/tridentrfid/timemachine/1/en/topic/tag-settings) manual. Once verified, the mechanics (Tag Timeout value, Normal vs. Virtual Line Best Seen method) still turned out to be more detail than the page needed and were cut entirely in favor of a link to Trident's docs — the same "keep to the key mechanism, link out for the rest" rule from the single-bullet-level note above applies even to detail that came from your own verification, not just detail copied from a source `CLAUDE.md`.
...
Lou also maintains a separate, older `loudevprocess` repo (cloned locally alongside the others, nested one level deeper: `...\projects\loudevprocess\loudevprocess`) — Sphinx/RST source for general development-process notes, published via readthedocs at [process.loutilities.com](https://process.loutilities.com/). It's mostly Linux/server account setup, database migration handling, Python package release procedure, Docker, and HTTPS (hence linked from `docs/infrastructure/hosting.md`'s Development section), but also covers development-environment setup and `loutilities`-specific `tables.py`/Editor coding patterns (hence also linked from `docs/framework/loutilities.md`). Unlike the app repos above, this isn't deepened into its own wiki page — just cross-linked from both, with a caution that parts predate the current Docker/Rocky Linux server setup and may be stale.
